 ...
 */
@Injectable()
export class PskAuthStrategy implements IAuthStrategy, OnApplicationBootstrap {
  ...
  /**
   * 应用启动完成后从 Redis 加载数据到内存
   * 在所有模块的 onModuleInit 完成后执行，确保 PskService 已经同步完成
   */
  ...
  async loadFromRedis() {
    try {
      ...
      this.sysLogger.log(`PSK loaded to memory cache: ${this.localCache.size} record(s)`)
    } catch (error) {
      this.sysLogger.log(LogMessages.PSK.LOAD_FROM_REDIS_FAILED(error.message))
    }
  }
