...
| 字段              | 类型    | 必填 | 说明                      |
| ----------------- | ------- | ---- | ------------------------- |
| `flashId`         | string  | 是   | 子设备ID                  |
| `online`          | boolean | 是   | **是否在线**              |
| `signal_strength` | number  | 否   | 信号强度（dBm，-255~255） |
| `battery_level`   | number  | 否   | 电池电量（0-100百分比）   |

### 后端处理

- 只更新消息中包含的子设备
- 不在消息中的子设备保持原状态不变
- 更新数据库：`online`, `signal_strength`, `battery_level`, `last_seen`
- 记录日志：`成功 X 个, 跳过 Y 个`
