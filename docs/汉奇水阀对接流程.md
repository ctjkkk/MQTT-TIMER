...
**不需要上报的字段（云端从产品配置表自动获取）：**

- ~~`deviceType`~~ - 从产品配置表获取
- ~~`capabilities`~~ - 从产品配置表获取出水口数量
- ~~`firmwareVersion`~~ - 使用产品配置的默认固件版本
- ~~`online`~~ - 新设备默认为在线
...
| `productId` | string | 是   | **涂鸦云产品ID**（字符串格式，云端根据这个字段查询产品配置表，获取出水口数量、设备类型等所有产品信息）例如："fdekfvdlkmqyslqr" |
...
### 后端处理（涂鸦 ProductID 方案）

**处理流程：**

1. **验证网关**：检查网关是否存在且已绑定用户
2. **校验字段**：检查 `uuid` 和 `productId` 是否存在
3. **查询产品配置**：根据 `productId` 查询产品配置表
   - 产品名称（name）
   - 设备类型（deviceType）
   - 出水口数量（outletCount）
   - 默认固件版本（defaultFirmwareVersion）
   - 默认电池电量（defaultBatteryLevel）
4. **创建/更新子设备**：
   - **已存在**：更新关联关系（userId, gatewayId, productId）
   - **不存在**：创建新记录，使用产品配置的默认值
5. **下发停止配对命令**：配对成功后自动下发

**好处：**

- 网关上报数据最简化（只有 uuid 和 productId）
- 产品信息完全由云端管理
- 新产品上线：云端加配置即可
- 固件升级、产品功能变更无需重新配对
...
- 子设备首次配对 → 创建新记录，使用产品配置的默认值
- 子设备重新配对 → 更新关联关系
- 子设备从A网关切换到B网关 → 更新 gatewayId
- 产品型号变更 → 更新 productId，云端自动识别新功能
...
- 只需上报 uuid 和 productId 两个字段
- productId 是**字符串类型**（例如："fdekfvdlkmqyslqr"）
